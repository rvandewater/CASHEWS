...
class AttnEncoder(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        attentions, input_encoded = (
            Variable(torch.zeros(input_data.size(0), self.seq_len, self.input_size)),
        ...
        for t in range(self.seq_len):
            x = torch.cat(
                (
                    ...
                    input_data.permute(0, 2, 1).to(device),
                ),
                dim=2,
            ).to(
                device
...
class Decoder(nn.Module):
    ...
    def forward(self, _, y_hist: torch.Tensor):
        ...
        """
        h_t, c_t = (init_hidden(y_hist, self.hidden_size), init_hidden(y_hist, self.hidden_size))
...
class AttnDecoder(nn.Module):
    ...
    def forward(self, input_encoded: torch.Tensor, y_history: torch.Tensor):
        ...
        for t in range(self.seq_len):
            ...
            x = tf.softmax(
                self.attn(x.view(-1, 2 * self.decoder_hidden_size + self.encoder_hidden_size)).view(
                    -1, self.seq_len
                ),
                dim=1,
            )
            ...
            y_tilde = self.fc(
                torch.cat((context.to(device), y_history[:, t].to(device)), dim=1)
            )  # (batch_size, out_size)
        ...
        return self.fc_out(
            torch.cat((h_t[0], context.to(device)), dim=1)
...
class AutoEncForecast(nn.Module):
    def __init__(self, config, input_size):
        ...
        super().__init__()
        self.encoder = (
            AttnEncoder(config, input_size).to(device)
            if config["input_att"]
            else Encoder(config, input_size).to(device)
        )
        self.decoder = (
            AttnDecoder(config).to(device) if config["temporal_att"] else Decoder(config).to(device)
        )
