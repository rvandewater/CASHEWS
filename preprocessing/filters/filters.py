...
def convert_to_ids(string_list):
    """
    Converts a list of strings with a prefix to integer IDs by stripping the prefix.

    Args:
        string_list (list): List of strings with a prefix.

    Returns:
        list: List of integer IDs.
    """
    return [int(s.split("-")[1]) for s in string_list]
